...
Return a JSON object with this exact structure:
{
  ...
  "overall_health": "Brief overall assessment"
    ...
    console.log(`Validating and analyzing ${bodyPart} image`);

    // Step 1: Validate if the image matches the selected body part
    const validationPrompt = `You are an image validation expert. Determine if this image shows a human ${bodyPart}. 
    ...
    Be strict - only return is_valid: true if the image clearly shows the specified body part (${bodyPart}).`;
    ...
    if (!validationResponse.ok) {
      ...
      console.error("AI Gateway validation error:", validationResponse.status, errorText);
      ...
      throw new Error(`AI Gateway validation error: ${validationResponse.status}`);
    }
    ...
    // If image doesn't match the body part, return error
    if (!validation.is_valid || validation.confidence < 0.6) {
      console.log(`Invalid image for ${bodyPart}: ${validation.reason}`);
      return new Response(
        JSON.stringify({ 
          error: "invalid_image",
          message: "This is not a valid image for this section. Please upload a clear image of your " + bodyPart + "."
        }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    // Step 2: Proceed with vitamin deficiency analysis
    console.log(`Image validated. Analyzing for vitamin deficiencies...`);
